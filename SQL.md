...
- `PRIMARY KEY`
    ...
    - A table can only have one primary key composed of single or multiple fields (_composite key_).
...
- _Table constraints_ are not tied to any particular column, and can encompass multiple columns.
    - `UNIQUE(column_list)`
...
- All candidate keys that are not primary keys are _secondary_ or _alternate_ keys.
...
- Implicitly triggered by `DELETE` or `UPDATE` operations _on the parent table_.
    - The database engine processes these actions _before_ firing any user-defined triggers (e.g., `AFTER` triggers).
- Use cascades
...
- Filters individual rows _before_ aggregation.
...
- **`BETWEEN`**
    - True if the operand falls within an _inclusive_ range.
...
- `HAVING` can be used to filter out groups that meet a certain condition.
    - Filters groups _after_ aggregation.
...
- The default sort order in `ORDER BY` clause is _ascending (ASC)_.
...
- **Use Cases**
    - _Logical Organization_ - Grouping related tables and other objects together.
    - _Access Control_ - Managing permissions and security at a granular level.
    - _Data Integrity_ - Enforcing rules and relationships between tables.
- **Components**
    - Tables
    - Views
    - Stored Procedures
...
- _Partial Indexes_ - includes only a subset of rows from a table
...
>     - It is the main _global_ object.
...
>     - Objects _within_ a schema must have unique names.
...
> [!note] Authentication
> _Peer authentication_ is the default authentication mechanism configured for most PostgreSQL installations. It assumes that the system administrator is also the database administrator.
